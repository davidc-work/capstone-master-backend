...
router.post("/transactions/sell", authenticate, async (req,res) => {
  ...
  console.log(req.body);
  let temp2 = await axios.delete(micro.url('profile', '/portfolio' + req.body.CustomerId + '/' + req.body.fundKey + '/' + req.body.quantity));
  if(!temp2) {
    return res.json({error: "something broke"})
  }
  let temp = await axios.post(micro.url('transactions', '/transactions/sell'), req.body).then(({ data }) => data).catch(err => err);
  console.log(temp)
  //Return data or response to frontend  
  res.json(temp)
});
